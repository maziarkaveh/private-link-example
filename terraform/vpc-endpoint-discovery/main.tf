...
# Optional: Validate the discovered NLB has the expected configuration
data "aws_lb_listener" "nlb_listeners" {
  count             = length(data.aws_lb.k8s_created_nlb.arn)
  load_balancer_arn = data.aws_lb.k8s_created_nlb.arn
  port              = 5432
}
